...
if (!rawData) {
  rawData = "{}";
  $persistentStore.write(rawData, "CheckinData") //初始化
}
let store = JSON.parse(rawData);
if (!store.fakerCheckin) {
  store.fakerCheckin = {
    "cookie": "",
    "updateTime": ""
  }
  $persistentStore.write(store, "CheckinData") //初始化
  console.log("请先在argument或persistentStore中添加cookie")
  $done()
}
if (!cookie) {
  cookie = store.fakerCheckin.cookie || $done()
}
